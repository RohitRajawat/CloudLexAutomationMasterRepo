Feature: Test the Add Lead Functionality
  ...
  Scenario: Check the basic details
       CampLejeune Mass tort should be selected default
       user select the toyota tort
        again select the Camp tort option
       Add new Lead option should be selected by default
        user select the existing lead option
        Search the existing lead with name "Rohit"
        select the lead "Rohit VCF" from existing list
